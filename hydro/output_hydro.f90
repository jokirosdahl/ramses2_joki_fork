...
!###################################################
!###################################################
recursive subroutine r_output_hydro(s,cpu_range,input_size,output_size,input_array)
  ...
  if(next_range>0)then
     call mdl_send_request(s%mdl,MDL_OUTPUT_HYDRO,next_cpu,next_range,input_size,output_size,input_array)
     call r_output_hydro(s,next_range,input_size,output_size,input_array)
  else
     filename=transfer(input_array,filename)
     call output_hydro(s%r,s%g,s%m,filename)
  endif
