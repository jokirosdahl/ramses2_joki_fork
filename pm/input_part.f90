...
subroutine m_input_part(pst)
  ...
  call r_mass_min_part(pst,pst%s%g%ncpu,1,2,pst%s%r%levelmin,output_array)
  
  ! Broadcast minimum particle mass
  call r_broadcast_mp_min(pst,pst%s%g%ncpu,2,0,output_array)
  
  ! Computing maximum particle count (only in master)
  call r_npart_max(pst,pst%s%g%ncpu,1,1,pst%s%r%levelmin,pst%s%p%npart_max)
...
!#####################################################################
!#####################################################################
recursive subroutine r_mass_min_part(pst,cpu_range,input_size,output_size,ilevel,output_array)
  use ramses_commons, only: pst_t
  use mdl_parameters
  implicit none
  type(pst_t)::pst
  ...
  if(next_range>0)then
     call mdl_send_request(pst%s%mdl,MDL_MASS_MIN_PART,next_cpu,next_range,input_size,output_size,ilevel)
     call r_mass_min_part(pst,next_range,input_size,output_size,ilevel,output_array)
...
!#####################################################################
!#####################################################################
recursive subroutine r_broadcast_mp_min(pst,cpu_range,input_size,output_size,input_array)
  use ramses_commons, only: pst_t
  use mdl_parameters
  implicit none
  type(pst_t)::pst
  ...
  if(next_range>0)then
     call mdl_send_request(pst%s%mdl,MDL_BROADCAST_MP_MIN,next_cpu,next_range,input_size,output_size,input_array)
     call r_broadcast_mp_min(pst,next_range,input_size,output_size,input_array)
  else
...
!#########################################################################
!#########################################################################
recursive subroutine r_npart_max(pst,cpu_range,input_size,output_size,ilevel,npart_max)
  use ramses_commons, only: pst_t
  use mdl_parameters
  implicit none
  type(pst_t)::pst
  ...
  if(next_range>0)then
     call mdl_send_request(pst%s%mdl,MDL_NPART_MAX,next_cpu,next_range,input_size,output_size,ilevel)
     call r_npart_max(pst,next_range,input_size,output_size,ilevel,npart_max)
     call mdl_get_reply(pst%s%mdl,next_cpu,output_size,next_npart_max)
