...
!###########################################################
!###########################################################
recursive subroutine r_star_formation(pst,ilevel,input_size)
  use mdl_module
...
!###########################################################
!###########################################################
subroutine star_formation(r,g,m,c,ilevel)
  use constants
...
#ifndef WITHOUTMPI
  integer::info
  integer,dimension(1:s%g%ncpu)::nsite_cpu_tot
#endif
  integer(kind=8),dimension(0:s%g%ncpu)::nsite_cum
  integer,dimension(1:s%g%ncpu)::nsite_cpu
  ...
  do ind=1,twotondim
     ...
     do igrid=m%head(ilevel),m%tail(ilevel),nvector
        ...
        ok = ok .and. m%grid(igrid)%uold(ind,1) > d0
  ...
  !---------------------------------------------------------
  ! Compute number of formation sites across all CPUs.
  !---------------------------------------------------------
  nsite_cpu=0
  nsite_cpu(myid)=nsite
#ifndef WITHOUTMPI
  call MPI_ALLREDUCE(nsite_cpu,nsite_cpu_tot,ncpu,MPI_INTEGER,MPI_SUM,MPI_COMM_WORLD,info)
  ...
  if(myid>1)then
     if(nsite_cum(myid-1)>0)then
        call RngStream_AdvanceState(g,0,nsite_cum(myid-1)))
     endif
  endif
