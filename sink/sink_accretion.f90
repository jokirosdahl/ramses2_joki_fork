...
subroutine sink_accretion(s,p,ilevel,macc_loc)
  ...
  real(dp)::d,e,ethermal,r2_sink,v_bondi,cs_sink,cs,rho_sink,vel,velocity
  real(dp)::weight
  ...
  do ipart=p%headp(ilevel),p%tailp(ilevel)
     ...
     dMBH_overdt = 4 * pi * rho_sink * r2_sink * v_bondi

     ! Eddinton accretion rate
     dMED_overdt = 4*pi*factG_in_cgs*p%mp(ipart)*mH/(0.1d0*sigma_T*c_cgs)*scale_t
     ...
     do j=1,nBHnei
        ...
        x_acc(1:ndim) = m_acc * xBHnei(1:ndim,j)
        ! Accreted relative momentum
        p_acc(1:ndim) = m_acc + v_rel(1:ndim)
