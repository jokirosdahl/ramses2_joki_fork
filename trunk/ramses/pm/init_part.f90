...
subroutine init_part_andreas
  ...
  allocate(ptcl_phi(npartmax))
#endif
  xp_andreas=0.0; vp_andreas=0.0; mp_andreas=0.0; levelp_andreas=0; idp_andreas=0
  ...
  if(nrestart>0)then
     ...
     ilun=2*ncpu+myid+10
     call title(nrestart,nchar)
     fileloc='output_'//TRIM(nchar)//'/part_andreas_'//TRIM(nchar)//'.out'
     call title(myid,nchar)
     fileloc=TRIM(fileloc)//TRIM(nchar)
     ...
     do idim=1,ndim
        read(ilun)xdp
        xp_andreas(1:npart2,idim)=xdp
     end do
     ! Read velocity
     do idim=1,ndim
        read(ilun)xdp
        vp_andreas(1:npart2,idim)=xdp
     end do
     ! Read mass
     read(ilun)xdp
     mp_andreas(1:npart2)=xdp
     deallocate(xdp)
     ! Read identity
     allocate(isp8(1:npart2))
     read(ilun)isp8
     idp_andreas(1:npart2)=isp8
     deallocate(isp8)
     ! Read level
     allocate(isp(1:npart2))
     read(ilun)isp
     levelp_andreas(1:npart2)=isp
     deallocate(isp)
     close(ilun)
     if(debug)write(*,*)'part.tmp read for processor ',myid
...
101        continue
           ...
           do 
              ...
              if(jpart >= start_ind(myid) .and. jpart < start_ind(myid+1))then
                 ...
                 xp_andreas(jpart_loc,1)=xx1+boxlen/2.0
                 xp_andreas(jpart_loc,2)=xx2+boxlen/2.0
                 xp_andreas(jpart_loc,3)=xx3+boxlen/2.0
                 vp_andreas(jpart_loc,1)=vv1
                 vp_andreas(jpart_loc,2)=vv2
...
100        continue
        ...
        if(debug)write(*,*)'npart_andreas=',npart_andreas,'/',npart_cpu(ncpu),jpart
...
end subroutine init_part_andreas
