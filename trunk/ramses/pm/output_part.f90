...
subroutine backup_part_andreas(filename)
  use amr_commons
  use pm_commons
  implicit none
  character(LEN=80)::filename
  ...
  write(ilun)dummyint
  ! Write position
  allocate(xdp(1:npart_andreas))
  do idim=1,ndim
     ipart=0
     do i=1,npartmax
        if(levelp_andreas(i)>0)then
           ipart=ipart+1
           xdp(ipart)=xp_andreas(i,idim)
         end if
     end do
     write(ilun)xdp
  end do
  ...
  do i=1,npartmax
     if(levelp_andreas(i)>0)then
        ipart=ipart+1
        xdp(ipart)=mp_andreas(i)
     end if
  end do
  write(ilun)xdp
  deallocate(xdp)
  ! Write identity
  allocate(ii8(1:npart_andreas))
  ipart=0
  do i=1,npartmax
     if(levelp_andreas(i)>0)then
        ipart=ipart+1
        ii8(ipart)=idp_andreas(i)
     end if
  end do
  write(ilun)ii8
  deallocate(ii8)
  ! Write level
  allocate(ll(1:npart_andreas))
  ipart=0
  do i=1,npartmax
     if(levelp_andreas(i)>0)then
        ipart=ipart+1
        ll(ipart)=levelp_andreas(i)
     end if
  end do
  write(ilun)ll
  deallocate(ll)
...
end subroutine backup_part_andreas
