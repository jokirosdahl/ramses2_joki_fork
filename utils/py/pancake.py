...
def d1(a):
    ...
    y12 = np.sqrt(y)
    integral, error = sp.integrate.quad(fy,1e-6,a)
    result = y12 / a * integral
    return result
...
def fpeebl(a):
    ...
    """
    y = omega_m * (1.0 / a - 1.0) + omega_l * (a**2 - 1.0) + 1.0
    fact, error = sp.integrate.quad(fy,1e-6,a)
    result = (omega_l * a**2 - 0.5 * omega_m / a) / y - 1.0 + a * fy(a) / fact
    return result
