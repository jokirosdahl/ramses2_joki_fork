...
ARGUMENTS:
  ...
  --movie-dir         Movie directory for map mode (e.g., movie1, movie2, or . for current dir)
...
def generate_map_frame(frame_num, map_file, args, frame_dir):
    ...
    # Run map2img
    try:
        # Convert relative path to absolute path for map2img
        # Always do this, even for default path
        if not os.path.isabs(map_file):
            # Get the current working directory where amr2vid.py was called from
            original_cwd = os.getcwd()
            absolute_path = os.path.abspath(map_file)
            # Update the map_file path in the command (it's at index 2)
            cmd[2] = absolute_path
...
def generate_frame(output_num, output_dir, args, frame_dir):
    ...
    # Run amr2img
    try:
        # Convert relative path to absolute path for amr2img
        # Always do this, even for default path
        path_to_use = args.path if args.path else "."
        if not os.path.isabs(path_to_use):
            # Get the current working directory where amr2vid.py was called from
            original_cwd = os.getcwd()
            absolute_path = os.path.abspath(path_to_use)
            # Find the index of --path in the command and update it
            if "--path" in cmd:
                path_index = cmd.index("--path")
                if path_index + 1 < len(cmd):
                    cmd[path_index + 1] = absolute_path
            else:
                # If no --path was provided, add it with the absolute path
                cmd.extend(["--path", absolute_path])
...
Examples:
  ...
  # Map mode examples:
  # Process density maps from movie1 directory (default)
  python amr2vid.py 1 100 --mode map --variable dens
  ...
  # Process temperature maps from movie1 subdirectory
  python amr2vid.py 1 100 --mode map --variable temp --movie-dir movie1
        """
    )
    ...
    parser.add_argument("--movie-dir", help="movie directory (e.g., movie1, movie2, or . for current dir) - only used in map mode, defaults to movie1")
    ...
    parser.add_argument("--path", help="specify a path (defaults to current directory)")
    ...
    if args.mode == "snapshot":
        ...
        if args.parallel:
            # Create full range and distribute work
            full_range = list(range(args.start, args.end + 1))
            my_outputs = [num for i, num in enumerate(full_range) if i % size == rank]
            print(f"Process {rank}: processing {len(my_outputs)} outputs from range {args.start}-{args.end}")
        else:
            # In serial mode, process the full requested range
            my_outputs = list(range(args.start, args.end + 1))
        
        # Generate frames
        generated_frames = []
        
        # Process the assigned outputs, skipping missing ones
        for i, output_num in enumerate(my_outputs):
            # Check if this output directory exists
            if args.parallel:
                # In parallel mode, we need to check the actual filesystem
                output_dir_path = os.path.join(args.path or ".", f"output_{output_num:05d}")
                if not os.path.exists(output_dir_path):
                    print(f"Warning: output directory output_{output_num:05d} doesn't exist")
                    continue
            else:
                # In serial mode, use the output_dirs list
            ...
            if frame_path:
                if args.parallel:
                    # In parallel mode, use temporary names to avoid conflicts
                    # We'll rename them sequentially at the end
                    temp_frame_filename = f"temp_frame_{rank:03d}_{output_num:05d}.png"
                    temp_frame_path = frame_dir / temp_frame_filename
                    
                    try:
                        os.rename(frame_path, temp_frame_path)
                        print(f"Generated temporary frame {temp_frame_filename} from output {output_num:05d}")
                        generated_frames.append(str(temp_frame_path))
                    except OSError as e:
                        print(f"Warning: Could not rename frame {output_num}: {e}")
                        generated_frames.append(frame_path)
                else:
                    # In serial mode, use simple sequential numbering
                    frame_number = i + 1
                    new_frame_filename = f"frame_{frame_number:05d}.png"
                    new_frame_path = frame_dir / new_frame_filename
                    
                    try:
                        os.rename(frame_path, new_frame_path)
                        print(f"Generated frame {frame_number:05d} from output {output_num:05d}")
        ...
        if args.parallel:
            ...
            if is_root:
                ...
                # Rename temporary frames to sequential frame numbers
                print("Renaming temporary frames to sequential numbers...")
                renamed_frames = []
                frame_counter = 1
                
                for temp_frame in generated_frames:
                    if "temp_frame_" in temp_frame:
                        # Extract output number from temp filename for better sorting
                        temp_name = os.path.basename(temp_frame)
                        # temp_frame_000_00001.png -> extract 00001
                        output_num = temp_name.split('_')[2].split('.')[0]
                        output_num = int(output_num)
                        
                        # Create new sequential frame name
                        new_frame_name = f"frame_{frame_counter:05d}.png"
                        new_frame_path = os.path.join(os.path.dirname(temp_frame), new_frame_name)
                        
                        try:
                            os.rename(temp_frame, new_frame_path)
                            print(f"Renamed {temp_name} -> {new_frame_name} (from output {output_num:05d})")
                            renamed_frames.append(new_frame_path)
                            frame_counter += 1
                        except OSError as e:
                            print(f"Warning: Could not rename {temp_name}: {e}")
                            renamed_frames.append(temp_frame)
                    else:
                        # Already properly named frame
                        renamed_frames.append(temp_frame)
                
                generated_frames = renamed_frames
        ...
        if is_root:
            ...
            if args.movie_dir is None:
                args.movie_dir = "movie1"  # Default to movie1 subdirectory
