...
!################################################################
!################################################################
recursive subroutine r_smooth_fine(r,g,m,p,mdl,cpu_range,input_size,output_size,ilevel,noct)
  ...
  if(next_range>0)then
     call mdl_send_request(mdl,MDL_SMOOTH_FINE,next_cpu,next_range,input_size,output_size,ilevel)
     call r_smooth_fine(r,g,m,p,mdl,next_range,input_size,output_size,ilevel,noct)
     call mdl_get_reply(mdl,next_cpu,output_size,next_noct)
     noct=noct+next_noct
  else
     call smooth_fine(r,g,m,ilevel,nflag)
     noct=nflag
  endif
...
!############################################################
!############################################################
subroutine smooth_fine(r,g,m,ilevel,nflag)
