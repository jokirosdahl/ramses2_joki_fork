!====================================================================
! Tracer particle seeding for Kelvin-Helmholtz instability test (3D)
!====================================================================
subroutine input_trac_khi_test(r,g,p,npart_tot)
  ...
  real(kind=8)::dx,xx1,xx2,xx3,zz,cell_vol,rho,vx,vz
  real(kind=8)::z_bound1,z_bound2,width_half,perturb_amplitude
  real(kind=8)::profile1,profile2,xx_norm,zz_norm
        ...
        do i1=0,n1-1
           do ipercell=1,n_per_cell
              if(ipart_global>=start_ind(g%myid).and.ipart_global<start_ind(g%myid+1))then
                 ...
                 ! Get normalized coordinates for perturbation calculation
                 xx_norm = p%xp(ipart,1)/r%boxlen
                 if(ndim>=3)then
                    zz_norm = p%xp(ipart,3)/r%boxlen
                 else
                    zz_norm = zz
                 endif
                 ...
                 if(ndim>=3)then
                    ...
                    if(abs(zz_norm - z_bound1) < width_half) then
                       profile1 = max(0.0d0, 1.0d0 - ((zz_norm - z_bound1)/width_half)**2)
                       vz = vz + perturb_amplitude * profile1 * sin(2.0d0*3.141592653589793d0*xx_norm)
                    endif
                    ! Profile centered at z = 2/3
                    if(abs(zz_norm - z_bound2) < width_half) then
                       profile2 = max(0.0d0, 1.0d0 - ((zz_norm - z_bound2)/width_half)**2)
                       vz = vz + perturb_amplitude * profile2 * sin(2.0d0*3.141592653589793d0*xx_norm)
                    endif
                    p%vp(ipart,3) = vz
                 endif
...
end subroutine input_trac_khi_test
