...
subroutine balance_part(s,p,ilevel)
  ...
  ! Load balance diagnostic variables
  integer,dimension(1:s%g%ncpu)::npart_final
  real(kind=8)::load_balance_ratio,load_balance_efficiency
  integer::npart_min,npart_max,npart_avg
  integer::npart_per_cell_max,npart_per_cell_avg
  real(kind=8)::particle_density_ratio
  ...
  !-------------------------------------
  ! Enhanced load balancing diagnostics
  !-------------------------------------
  if(r%verbose_load_balance.and.g%myid==1)then
     write(*,'("=== Load Balance Statistics ===")')
     write(*,'("Particles to send/receive: ",I8,"/",I8)')send_cnt_tot,recv_cnt_tot
     write(*,'("Local particles after balance: ",I8)')count_loc+recv_cnt_tot
     write(*,'("===============================")')
  endif
