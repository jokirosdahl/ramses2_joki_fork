...
!!! Advanced Sink routines
#ifdef BZ_sink
      !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
      ! ISM -> Disc reservoir accretion
      !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
!!! Basic Sink routines
      ...
      !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
      ! Internal sink evolution
      !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
      call evolve_internal_sink(s,p,ipart,m_acc,x_acc,p_acc,l_acc)
...
subroutine evolve_internal_sink(s,p,ipart,m_acc,x_acc,p_acc,l_acc)
   ...
   real(dp),dimension(1:ndim)::x_acc,p_acc,l_acc
   !==================================================================
   ! This is the RAMSES routine for internal sink evolution, updating
   ! sink properties based on accreted quantities.
   ! Written by Nicholas Choustikov (Apr 2025)
   !==================================================================
   ...
   if(.not.s%r%fix_sink_mass)p%mp(ipart) =   p%mp(ipart) + m_acc

   !if(s%r%verbose_sink)then
...
end subroutine evolve_internal_sink
...
subroutine sink_accretion(s,p,ilevel,ipart,dx_loc,vol_loc,nBHnei,scale_l,scale_t,scale_d,factG,lambda_sonic,dMBH_overdt,dMEd_overdt,rho_inf,cs_gas,m_acc,x_acc,p_acc,l_acc)
   use constants
   use amr_parameters, only: ndim,twotondim,dp
   use hydro_parameters, only: nvar, nener
   use amr_commons, only: nbor,oct
   ...
   real(dp)::dMBH_overdt,dMEd_overdt,rho_inf,cs_gas
   real(dp)::m_acc
   real(dp),dimension(1:ndim)::x_acc,p_acc,l_acc
   ...
   real(dp),dimension(1:ndim)::vv,v_rel,vel_gas
...
subroutine AGN_feedback(s,p,ilevel,ipart,dx_loc,vol_loc,nBH_fb_nei,scale_v,dMBH_overdt,dMEd_overdt,tan_theta,fbk_mass_agn,fbk_mom_agn,fbk_ener_agn)
   ...
   if(ok_blast_agn)then
      ...
      if(acc_ratio.gt.r%agn_fbk_mode_switch_threshold)then
         ...
         !fbk_ener_agn = r%epsilon_therm_jet*r%epsilon_rad*jet_mass/r%kin_mass_loading*(c_cgs/scale_v)**2
      end if
