...
#ifdef BZ_sink
        ! Swap BH masses
        if(allocated(p%mBH))then
        mp_tmp=p%mBH(ipart)
        p%mBH(ipart)=p%mBH(jpart)
        p%mBH(jpart)=mp_tmp
        end if

        ! Swap Disc masses
        if(allocated(p%mD))then
        mp_tmp=p%mD(ipart)
        p%mD(ipart)=p%mD(jpart)
        p%mD(jpart)=mp_tmp
        end if
#endif
        ! Swap metallicity
...
#ifdef BZ_sink
        ! Swap Disc angular momentum
        if(allocated(p%jD))then
           jp_tmp(1:ndim)=p%jD(ipart,1:ndim)
           p%jD(ipart,1:ndim)=p%jD(jpart,1:ndim)
           p%jD(jpart,1:ndim)=jp_tmp(1:ndim)
        end if
#endif
        ! Swap birth time
...
#ifdef BZ_sink
  !-------------------------
  ! Swap Disc Angular momenta (spins)
  !-------------------------
  if(allocated(p%jD))then
     do idim=1,ndim
