...
| `riemann2d=’none’`    |  `Characher LEN=20`| Name of the 2D Riemann solver for the induction equation (`MHD=1` only). Possible choices are `llf`, `hll`, `roe`,`hlla`, `hllf`, `hlld` and `upwind`. If set to `none`, use the same as the 1D Riemann solver. |
...
| `slope_mag_type=1` |  `Integer`    | Type of slope limiter used for the piecewise linear reconstruction of face-averaged quantities (`MHD=1` only): `slope_type=0`: first order scheme, `slope_type=1`: MinMod limiter, `slope_type=2`: MonCen limiter. If not set, then the code will use `slope_mag_type=slope_type`.|
