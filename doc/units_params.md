...
| `units_time`        | `real`  | 1 | This is the scaling factor used to convert times from code units to cgs units. Units are therefore [s]. When `poisson=.true.`, units of time must be set so that `G=1`. If you are not sure, used predefined unit systems at compilation time using for example `UNITS=COSMO` or `UNITS=MERGER`. Units are defined in the file `amr/units.f90`. |
